...
async function fetchData(url, id, method = "GET", body = {}) {
    try {
        const response = await fetch(id !== undefined ? `${url}/${id}` : url, method === "GET" ? {method} : {
            method,
        ...
        <>
            <div className={`StatsContMain ${colorTheme.darkContBackground}`}>
                ...
                {userStats ? (
                    <>
                        <div
                            className={`generalStatsCont statsGrid1 ${colorTheme.lightContBackground} ${colorTheme.darkText}`}>
                            <div>
                            ...
                            <div>
                                ...
                                <div className="progressCont">
                                    <CircularProgressbarWithChildren
                                        ...
                                        {<div><p className="donutTxt">{`${userStats.stats[1].category.name}:`}
                                            <br/> {`${userStats.stats[1].category.points}`}</p></div>}
                                    </CircularProgressbarWithChildren>
                                </div>
                        ...
                        <div className="XPContRemaining statsGrid1">
                            {filteredUserStats ?
                                <div>
                                    <select name="category" className="categoryDropStats" onChange={(e) => {
                                        setSelectedCat(e.target.value);
                                        setSelectedCatPoints(e.target.id)
                                    }}> {filteredUserStats[0].name
                                    }
                                        {filteredUserStats
                                            .sort((a, b) => console.log(a.category.name, b.category.name))
                                            .map(cat => (
                                                <option key={cat._id} id={cat.category.points}
                                                        value={cat.category.name}>{cat.category.name}</option>
                                            ))}
                                    </select>
                        ...
                        {mostQuestionsCat ? <div
                            className={`generalStatsCont statsGrid2 ${colorTheme.lightContBackground} ${colorTheme.darkText}`}>
                            <div className="row1">
